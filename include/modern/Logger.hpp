...
#define LOG_TRACE(category, message)                                                              \
    ::openauto::modern::Logger::getInstance().trace(::openauto::modern::LogCategory::category,    \
                                                    typeid(*this).name(), __FUNCTION__, __FILE__, \
                                                    __LINE__, message)

#define LOG_DEBUG(category, message)                                                              \
    ::openauto::modern::Logger::getInstance().debug(::openauto::modern::LogCategory::category,    \
                                                    typeid(*this).name(), __FUNCTION__, __FILE__, \
                                                    __LINE__, message)

#define LOG_INFO(category, message)                                                              \
    ::openauto::modern::Logger::getInstance().info(::openauto::modern::LogCategory::category,    \
                                                   typeid(*this).name(), __FUNCTION__, __FILE__, \
                                                   __LINE__, message)

#define LOG_WARN(category, message)                                                              \
    ::openauto::modern::Logger::getInstance().warn(::openauto::modern::LogCategory::category,    \
                                                   typeid(*this).name(), __FUNCTION__, __FILE__, \
                                                   __LINE__, message)

#define LOG_ERROR(category, message)                                                              \
    ::openauto::modern::Logger::getInstance().error(::openauto::modern::LogCategory::category,    \
                                                    typeid(*this).name(), __FUNCTION__, __FILE__, \
                                                    __LINE__, message)

#define LOG_FATAL(category, message)                                                              \
    ::openauto::modern::Logger::getInstance().fatal(::openauto::modern::LogCategory::category,    \
                                                    typeid(*this).name(), __FUNCTION__, __FILE__, \
                                                    __LINE__, message)
...
#define SLOG_TRACE(category, component, message)                                                 \
    ::openauto::modern::Logger::getInstance().trace(::openauto::modern::LogCategory::category,   \
                                                    component, __FUNCTION__, __FILE__, __LINE__, \
                                                    message)
...
#define LOG_TRACE_STREAM(category, stream)                                                         \
    do {                                                                                           \
        ...
        oss_ << stream;                                                                            \
        ::openauto::modern::Logger::getInstance().trace(::openauto::modern::LogCategory::category, \
                                                        "Component", __FUNCTION__, __FILE__,       \
                                                        __LINE__, oss_.str());                     \
    } while (0)
...
#define LOG_PERF_START(name) auto perf_start_##name = std::chrono::high_resolution_clock::now()
...
#define LOG_PERF_END(category, name)                                                       \
    do {                                                                                   \
        ...
        auto perf_duration_##name = std::chrono::duration_cast<std::chrono::microseconds>( \
                                        perf_end_##name - perf_start_##name)               \
                                        .count();                                          \
...
#define OPENAUTO_LOG(severity)                                                                   \
    ::openauto::modern::Logger::getInstance().info(::openauto::modern::LogCategory::GENERAL,     \
...
class LogStream {
  public:
    LogStream(::openauto::modern::LogLevel level, ::openauto::modern::LogCategory category,
              const std::string& component, const std::string& function, const std::string& file,
              int line)
        : level_(level),
          category_(category),
    ...
    ~LogStream() {
        openauto::modern::Logger::getInstance().log(level_, category_, component_, function_, file_,
                                                    line_, oss_.str());
    }
...
#define OPENAUTO_LOG_STREAM(severity, category)                                          \
    ::openauto::modern::LogStream(::openauto::modern::LogLevel::severity,                \
                                  ::openauto::modern::LogCategory::category, "OpenAuto", \
