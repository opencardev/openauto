...
TEST_F(LoggerTest, StreamMacrosTest) {
    ...
    EXPECT_NO_THROW(LOG_DEBUG_STREAM(LogCategory::ANDROID_AUTO,
                                     "Stream test: " << testValue << " number: " << numValue));
    ...
    EXPECT_NO_THROW(
        LOG_ERROR_STREAM(LogCategory::NETWORK, "Error stream: " << testValue << " " << numValue));
}
...
TEST_F(LoggerTest, ContextTest) {
    ...
    std::map<std::string, std::string> context = {{"session_id", "test_session_123"},
                                                  {"user_id", "test_user"},
                                                  {"component", "test_component"}};

    EXPECT_NO_THROW(logger.logWithContext(LogLevel::INFO, LogCategory::SYSTEM, "TestComponent",
                                          "TestFunction", "test.cpp", 123,
