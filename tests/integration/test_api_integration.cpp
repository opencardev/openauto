...
TEST_F(APIIntegrationTest, StateAPITest) {
    ...
    auto transitionResponse =
        client->Post("/api/v1/state/transition", transitionData.dump(), "application/json");
...
TEST_F(APIIntegrationTest, EventAPITest) {
    ...
    // Subscribe to events
    eventBus->subscribe(EventType::CONNECTION_STATUS,
...
TEST_F(APIIntegrationTest, ErrorHandlingTest) {
    ...
    auto invalidTransitionResponse =
        client->Post("/api/v1/state/transition", invalidTransition.dump(), "application/json");
