...
TEST_F(ConfigurationIntegrationTest, StateMachineIntegrationTest) {
    ...
    eventBus->subscribe(EventType::CONFIG_CHANGED, [&configEvent](std::shared_ptr<Event> event) {
        configEvent = event;
    });
    
    eventBus->subscribe(EventType::STATE_CHANGED, [&stateEvent](std::shared_ptr<Event> event) {
        stateEvent = event;
    });
...
TEST_F(ConfigurationIntegrationTest, ChangePropagationTest) {
    ...
    eventBus->subscribe(EventType::CONFIG_CHANGED, [&events](std::shared_ptr<Event> event) {
    ...
    std::unordered_map<std::string, ConfigValue> bulkChanges = {
        {"bulk.test.1", std::string("bulk_value1")},
        {"bulk.test.2", 999},
        {"bulk.test.3", false}
    };
