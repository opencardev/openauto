...
class EndToEndTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        apiServer = std::make_unique<RestApiServer>(eventBus, configManager, stateMachine,
                                                    "127.0.0.1", 18083);
        modernIntegration =
            std::make_unique<ModernIntegration>(eventBus, configManager, stateMachine);
...
TEST_F(EndToEndTest, AndroidAutoConnectionScenarioTest) {
    ...
    auto handshakeEvent =
        std::make_shared<Event>(EventType::CONNECTION_STATUS, "android_auto_entity");
...
TEST_F(EndToEndTest, RuntimeConfigurationChangeScenarioTest) {
    ...
    eventBus->subscribe(
        ...
        [&configChangeEvents](std::shared_ptr<Event> event) { configChangeEvents++; });
...
TEST_F(EndToEndTest, HighLoadScenarioTest) {
    ...
    eventBus->subscribe(
        EventType::CONFIG_CHANGED,
        [&totalEventsProcessed](std::shared_ptr<Event> event) { totalEventsProcessed++; });
    eventBus->subscribe(
...
TEST_F(EndToEndTest, GracefulShutdownScenarioTest) {
    ...
    EXPECT_EQ(shutdownConfigManager->getValue<std::string>("shutdown.test.value", ""),
              "before_shutdown");
...
TEST_F(EndToEndTest, SystemRestartScenarioTest) {
    ...
    EXPECT_EQ(newConfigManager->getValue<std::string>("restart.test.persist", ""),
              "persistent_value");
    EXPECT_EQ(newConfigManager->getValue<int>("restart.test.number", 0), 999);

    // Start new API server
    auto newApiServer = std::make_unique<RestApiServer>(eventBus, newConfigManager, stateMachine,
                                                        "127.0.0.1", 18084);
    newApiServer->start();
