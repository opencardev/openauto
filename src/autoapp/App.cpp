...
void App::waitForUSBDevice() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        try {
            ...
            LOG_ERROR(GENERAL,
                      "[App] waitForUSBDevice() -exception caused by this->waitForDevice()");
        }
        try {
            this->enumerateDevices();
        } catch (...) {
            LOG_ERROR(GENERAL,
                      "[App] waitForUSBDevice() exception caused by this->enumerateDevices()");
        }
    });
}
    ...
    strand_.dispatch([this, self = this->shared_from_this(), socket = std::move(socket)]() mutable {
        ...
        if (androidAutoEntity_ != nullptr) {
            ...
            try {
                ...
                LOG_ERROR(
                    UI, "[App] onAndroidAutoQuit: exception caused by androidAutoEntity_->stop()");
            }
            try {
                androidAutoEntity_.reset();
            } catch (...) {
                LOG_ERROR(
                    UI, "[App] onAndroidAutoQuit: exception caused by androidAutoEntity_.reset()");
            }
        }
        ...
        try {
            ...
            auto tcpEndpoint(
                std::make_shared<aasdk::tcp::TCPEndpoint>(tcpWrapper_, std::move(socket)));
...
void App::stop() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        ...
        try {
            ...
            LOG_ERROR(GENERAL,
                      "[App] stop: exception caused by connectedAccessoriesEnumerator_->cancel()");
        }
        try {
            usbHub_->cancel();
        } catch (...) {
...
void App::startServerSocket() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        ...
        acceptor_.async_accept(
            *socket, std::bind(&App::handleNewClient, this, socket, std::placeholders::_1));
    });
}

void App::handleNewClient(std::shared_ptr<boost::asio::ip::tcp::socket> socket,
