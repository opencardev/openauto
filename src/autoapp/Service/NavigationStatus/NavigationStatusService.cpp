...
NavigationStatusService::NavigationStatusService(boost::asio::io_service &ioService,
    ...
    : strand_(ioService),
      timer_(ioService),
      channel_(std::make_shared<aasdk::channel::navigationstatus::NavigationStatusService>(
          strand_, std::move(messenger))) {}

void NavigationStatusService::start() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        LOG_INFO(ANDROID_AUTO, "[NavigationStatusService] start()");
    });
}

void NavigationStatusService::stop() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        LOG_INFO(ANDROID_AUTO, "[NavigationStatusService] stop()");
    });
}

void NavigationStatusService::pause() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        LOG_INFO(ANDROID_AUTO, "[NavigationStatusService] pause()");
    });
}

void NavigationStatusService::resume() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
        LOG_INFO(ANDROID_AUTO, "[NavigationStatusService] resume()");
    });
}

void NavigationStatusService::fillFeatures(
    aap_protobuf::service::control::message::ServiceDiscoveryResponse &response) {
    LOG_INFO(ANDROID_AUTO, "[NavigationStatusService] fillFeatures()");
...
void NavigationStatusService::onChannelOpenRequest(
    ...
    LOG_INFO(ANDROID_AUTO,
             "[NavigationStatusService] Channel Id: " + std::to_string(request.service_id()) +
    ...
    promise->then([]() {}, std::bind(&NavigationStatusService::onChannelError,
...
void NavigationStatusService::onChannelError(const aasdk::error::Error &e) {
    LOG_ERROR(ANDROID_AUTO, "[NavigationStatusService] onChannelError(): " + std::string(e.what()));
}
}  // namespace f1x::openauto::autoapp::service::navigationstatus
