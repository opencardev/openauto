...
namespace f1x::openauto::autoapp::service::sensor {
  ...
  void SensorService::start() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      if (gps_open("127.0.0.1", "2947", &this->gpsData_)) {
        OPENAUTO_LOG_WARN(ANDROID_AUTO, "[SensorService] can't connect to GPSD.");
      } else {
        OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] Connected to GPSD.");
      ...
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] start()");
  ...
  void SensorService::stop() {
    ...
    strand_.dispatch([this, self = this->shared_from_this()]() {
      ...
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] stop()");
    });
  }

  void SensorService::pause() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] pause()");
    });
  }

  void SensorService::resume() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] resume()");
    });
  }

  void SensorService::fillFeatures(
      aap_protobuf::service::control::message::ServiceDiscoveryResponse &response) {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] fillFeatures()");
  ...
  void SensorService::onChannelOpenRequest(const aap_protobuf::service::control::message::ChannelOpenRequest &request) {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] onChannelOpenRequest()");
    OPENAUTO_LOG_DEBUG(ANDROID_AUTO, (std::stringstream() << "[SensorService] Channel Id: " << request.service_id() << ", Priority: "
  ...
  void SensorService::onSensorStartRequest(
    ...
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] onSensorStartRequest()");
    OPENAUTO_LOG_DEBUG(ANDROID_AUTO, (std::stringstream() << "[SensorService] Request Type: " << request.type()).str());
  ...
  void SensorService::sendDrivingStatusUnrestricted() {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] sendDrivingStatusUnrestricted()");
  ...
  void SensorService::sendNightData() {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] sendNightData()");
    aap_protobuf::service::sensorsource::message::SensorBatch indication;

    if (SensorService::isNight) {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] Night Mode Triggered");
      indication.add_night_mode_data()->set_night_mode(true);
    } else {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] Day Mode Triggered");
  ...
  void SensorService::sendGPSLocationData() {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] sendGPSLocationData()");
    aap_protobuf::service::sensorsource::message::SensorBatch indication;
  ...
  void SensorService::sensorPolling() {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] sensorPolling()");
    if (!this->stopPolling) {
      strand_.dispatch([this, self = this->shared_from_this()]() {
  ...
  bool SensorService::is_file_exist(const char *fileName) {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[SensorService] is_file_exist()");
    std::ifstream ifile(fileName, std::ios::in);
    return ifile.good();
  }

  void SensorService::onChannelError(const aasdk::error::Error &e) {
    OPENAUTO_LOG_ERROR(ANDROID_AUTO, (std::stringstream() << "[SensorService] onChannelError(): " << e.what()).str());
  }
}
