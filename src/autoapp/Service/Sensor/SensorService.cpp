...
*
*   void SensorService::onChannelOpenRequest(const
aap_protobuf::service::control::message::ChannelOpenRequest &request) { LOG_INFO(ANDROID_AUTO,
"[SensorService] onChannelOpenRequest()"); LOG_DEBUG(ANDROID_AUTO, "[SensorService] Channel Id: " +
std::to_string(request.service_id()) + ", Priority: " + std::to_string(request.priority()));

    aap_protobuf::service::control::message::ChannelOpenResponse response;uto is free software: you
...
void SensorService::onChannelOpenRequest(
    ...
    LOG_DEBUG(ANDROID_AUTO, "[SensorService] Channel Id: " + std::to_string(request.service_id()) +
    ...
    promise->then([]() {}, std::bind(&SensorService::onChannelError, this->shared_from_this(),
                                     std::placeholders::_1));
...
#endif
            if ((this->gpsEnabled_) && (gps_waiting(&this->gpsData_, 0)) &&
                (gpsDataAvailable == true) &&
                (this->gpsData_.fix.mode == MODE_2D || this->gpsData_.fix.mode == MODE_3D) &&
                (this->gpsData_.set & TIME_SET) && (this->gpsData_.set & LATLON_SET)) {
                this->sendGPSLocationData();
            }

            timer_.expires_from_now(boost::posix_time::milliseconds(250));
            timer_.async_wait(
                strand_.wrap(std::bind(&SensorService::sensorPolling, this->shared_from_this())));
        });
    }
}
