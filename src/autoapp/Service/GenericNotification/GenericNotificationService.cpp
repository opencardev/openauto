...
namespace f1x::openauto::autoapp::service::genericnotification {
  ...
  void GenericNotificationService::onChannelOpenRequest(
    ...
    LOG_DEBUG(ANDROID_AUTO, ("[GenericNotificationService] Channel Id: " + std::to_string(request.service_id()) + ", Priority: " + std::to_string(request.priority())).c_str());
  ...
  void GenericNotificationService::onChannelError(const aasdk::error::Error &e) {
    LOG_ERROR(ANDROID_AUTO, ("[GenericNotificationService] onChannelError(): " + std::string(e.what())).c_str());
  }
}
