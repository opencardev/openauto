...
namespace service {
namespace mediasink {
VideoMediaSinkService::VideoMediaSinkService(
...
void VideoMediaSinkService::fillFeatures(
    ...
    LOG_INFO(ANDROID_AUTO, "[VideoMediaSinkService] getVideoResolution " +
                               VideoCodecResolutionType_Name(videoOutput_->getVideoResolution()));
    LOG_INFO(ANDROID_AUTO, "[VideoMediaSinkService] getVideoFPS " +
    ...
    LOG_INFO(ANDROID_AUTO,
             "[VideoMediaSinkService] height " + std::to_string(videoMargins.height()));
    LOG_INFO(ANDROID_AUTO, "[VideoMediaSinkService] getScreenDPI " +
                               std::to_string(videoOutput_->getScreenDPI()));
}
...
void VideoMediaSinkService::onMediaChannelSetupRequest(
    ...
    promise->then(
        std::bind(&VideoMediaSinkService::sendVideoFocusIndication, this->shared_from_this()),
...
void VideoMediaSinkService::onChannelOpenRequest(
    ...
    LOG_INFO(ANDROID_AUTO,
             "[VideoMediaSinkService] Channel Id: " + std::to_string(request.service_id()) +
                 ", Priority: " + std::to_string(request.priority()));

    const aap_protobuf::shared::MessageStatus status =
        videoOutput_->open() ? aap_protobuf::shared::MessageStatus::STATUS_SUCCESS
    ...
    auto promise = aasdk::channel::SendPromise::defer(strand_);
    promise->then([]() {}, std::bind(&VideoMediaSinkService::onChannelError,
...
void VideoMediaSinkService::onMediaWithTimestampIndication(
...
void VideoMediaSinkService::onChannelError(const aasdk::error::Error &e) {
    LOG_ERROR(ANDROID_AUTO,
              "[VideoMediaSinkService] onChannelError(): " + std::string(e.what()) +
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    LOG_INFO(ANDROID_AUTO,
             "[VideoMediaSinkService] Display index: " + std::to_string(request.disp_channel_id()) +
