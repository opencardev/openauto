...
AndroidAutoEntity::~AndroidAutoEntity() { OPENAUTO_LOG(debug) << "[AndroidAutoEntity] destroy."; }
...
void AndroidAutoEntity::onServiceDiscoveryRequest(
    ...
    promise->then([]() {}, std::bind(&AndroidAutoEntity::onChannelError, this->shared_from_this(),
                                     std::placeholders::_1));
...
void AndroidAutoEntity::onAudioFocusRequest(
     ...
     * STATE_LOSS when vehicle is playing high priority sound after stopping native media (ie USB,
     * RADIO)
     *
...
void AndroidAutoEntity::onNavigationFocusRequest(
    ...
    /*
     * If the MD sends NAV_FOCUS_PROJECTED in the request, we should stop any local navigation on
     * the HU and grant NAV_FOCUS_NATIVE in the response. If the HU starts its own Nav, we should
     * send NAV_FOCUS_NATIVE.
     *
     * For now, this is fine to be hardcoded as OpenAuto does not provide any local navigation, only
     * that provided through Android Auto.
     */
