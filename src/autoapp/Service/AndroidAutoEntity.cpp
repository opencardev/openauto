...
*  it under the terms of the GNU General Publ        void AndroidAutoEntity::onAudioFocusRequest(
          ...
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] NavFocusRequestNotification type received: " << NavFocusType_Name(request.focus_type()));const aap_protobuf::service::control::message::AudioFocusRequest &request) {
          LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] onAudioFocusRequest()");
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] AudioFocusRequestType received: "
                             << AudioFocusRequestType_Name(request.audio_focus_type()));cense as published by
    ...
    namespace autoapp {
      namespace service {
        ...
        AndroidAutoEntity::~AndroidAutoEntity() {
          LOG_DEBUG(ANDROID_AUTO, "[AndroidAutoEntity] destroy.");
        }
        ...
        void AndroidAutoEntity::start(IAndroidAutoEntityEventHandler &eventHandler) {
          strand_.dispatch([this, self = this->shared_from_this(), eventHandler = &eventHandler]() {
            LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] start()");
            ...
            LOG_DEBUG(ANDROID_AUTO, "[AndroidAutoEntity] Send Version Request.");
        ...
        void AndroidAutoEntity::stop() {
          strand_.dispatch([this, self = this->shared_from_this()]() {
            LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] stop()");
            ...
            try {
              ...
              LOG_ERROR(ANDROID_AUTO, "[AndroidAutoEntity] stop() - exception when stopping.");
            }
          });
        }
        ...
        void AndroidAutoEntity::pause() {
          strand_.dispatch([this, self = this->shared_from_this()]() {
            LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] pause()");

            try {
              std::for_each(serviceList_.begin(), serviceList_.end(),
                            std::bind(&IService::pause, std::placeholders::_1));
            } catch (...) {
              LOG_ERROR(ANDROID_AUTO, "[AndroidAutoEntity] pause() - exception when pausing.");
            }
          });
        }
        ...
        void AndroidAutoEntity::resume() {
          strand_.dispatch([this, self = this->shared_from_this()]() {
            LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] resume()");

            try {
              std::for_each(serviceList_.begin(), serviceList_.end(),
                            std::bind(&IService::resume, std::placeholders::_1));
            } catch (...) {
              LOG_ERROR(ANDROID_AUTO, "[AndroidAutoEntity] resume() exception when resuming.");
            }
          });
        }
        ...
        void AndroidAutoEntity::onVersionResponse(uint16_t majorCode, uint16_t minorCode,
          ...
          LOG_INFO_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] Version Received: " << majorCode << "." << minorCode
                             << ", with status: " << status);

          if (status == aap_protobuf::shared::MessageStatus::STATUS_NO_COMPATIBLE_VERSION) {
            LOG_ERROR(ANDROID_AUTO, "[AndroidAutoEntity] Version mismatch.");
            this->triggerQuit();
          } else {
            LOG_DEBUG(ANDROID_AUTO, "[AndroidAutoEntity] Version matches.");

            try {
              LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] Beginning SSL handshake.");
              ...
              LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] Handshake Error.");
        ...
        void AndroidAutoEntity::onHandshake(const aasdk::common::DataConstBuffer &payload) {
          ...
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] Payload size: " << payload.size());

          try {
            cryptor_->writeHandshakeBuffer(payload);

            if (!cryptor_->doHandshake()) {
              LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] Re-attempting handshake.");
              ...
              LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] Handshake completed.");
            ...
            LOG_ERROR(ANDROID_AUTO, "[AndroidAutoEntity] Error during handshake");
            this->onChannelError(e);
          }
        }
        ...
        void AndroidAutoEntity::onServiceDiscoveryRequest(
          ...
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] Type: " << request.label_text() << ", Model: "
                             << request.device_name());
        ...
        void AndroidAutoEntity::onAudioFocusRequest(
          ...
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] AudioFocusStateType determined: "
        ...
        void AndroidAutoEntity::onByeByeRequest(
          ...
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] Reason received: " << request.reason());
        ...
        void AndroidAutoEntity::onByeByeResponse(
          ...
          LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] onByeByeResponse()");
          this->triggerQuit();
        }
        ...
        void AndroidAutoEntity::onPingResponse(const aap_protobuf::service::control::message::PingResponse &response) {
          ...
          LOG_DEBUG_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] Timestamp: " << response.timestamp());
          pinger_->pong();
          controlServiceChannel_->receive(this->shared_from_this());
        }

        void AndroidAutoEntity::onChannelError(const aasdk::error::Error &e) {
          LOG_FATAL_STREAM(ANDROID_AUTO, "[AndroidAutoEntity] onChannelError(): " << e.what());
          this->triggerQuit();
        }

        void AndroidAutoEntity::triggerQuit() {
          LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] triggerQuit()");
        ...
        void AndroidAutoEntity::schedulePing() {
          LOG_INFO(ANDROID_AUTO, "[AndroidAutoEntity] schedulePing()");
          ...
          promise->then([this, self = this->shared_from_this()]() {
                        ...
                        [this, self = this->shared_from_this()](auto error) {
                          if (error != aasdk::error::ErrorCode::OPERATION_ABORTED &&
                            ...
                            LOG_ERROR(ANDROID_AUTO, "[AndroidAutoEntity] Ping timer exceeded.");
                            this->triggerQuit();
                          }
                        });
        ...
        void AndroidAutoEntity::sendPing() {
          LOG_DEBUG(ANDROID_AUTO, "[AndroidAutoEntity] sendPing()");
          auto promise = aasdk::channel::SendPromise::defer(strand_);
          promise->then([]() {},
                        std::bind(&AndroidAutoEntity::onChannelError, this->shared_from_this(), std::placeholders::_1));
