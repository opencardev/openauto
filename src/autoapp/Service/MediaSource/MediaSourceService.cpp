...
namespace f1x::openauto::autoapp::service::mediasource {
  ...
  void MediaSourceService::start() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] start()");
  ...
  void MediaSourceService::stop() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] stop()");
      audioInput_->stop();
    });
  }
  ...
  void MediaSourceService::pause() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] pause()");
    });
  }

  void MediaSourceService::resume() {
    strand_.dispatch([this, self = this->shared_from_this()]() {
      OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] resume()");
    });
  }
  ...
  void MediaSourceService::fillFeatures(
    ...
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] fillFeatures()");
  ...
  MediaSourceService::onChannelOpenRequest(const aap_protobuf::service::control::message::ChannelOpenRequest &request) {
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] onChannelOpenRequest()");
    OPENAUTO_LOG_INFO(ANDROID_AUTO, (std::stringstream() << "[MediaSourceService] Channel Id: " << request.service_id() << ", Priority: "
    ...
    OPENAUTO_LOG_INFO(ANDROID_AUTO, (std::stringstream() << "[MediaSourceService] Status determined: "
  ...
  void MediaSourceService::onChannelError(const aasdk::error::Error &e) {
    OPENAUTO_LOG_ERROR(ANDROID_AUTO, (std::stringstream() << "[MediaSourceService] onChannelError(): " << e.what()).str());
  }
  ...
  MediaSourceService::onMediaChannelSetupRequest(const aap_protobuf::service::media::shared::message::Setup &request) {
    ...
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] onMediaChannelSetupRequest()");
    OPENAUTO_LOG_INFO(ANDROID_AUTO, (std::stringstream() << "[MediaSourceService] Channel Id: " << aasdk::messenger::channelIdToString(channel_->getId())
                       << ", Codec: " << MediaCodecType_Name(request.type())).str());
  ...
  void MediaSourceService::onMediaChannelAckIndication(
    ...
    OPENAUTO_LOG_DEBUG(ANDROID_AUTO, "[MediaSourceService] onMediaChannelAckIndication()");
    channel_->receive(this->shared_from_this());
  }
  ...
  void MediaSourceService::onMediaSourceOpenRequest(
    ...
    OPENAUTO_LOG_INFO(ANDROID_AUTO, "[MediaSourceService] onMediaSourceOpenRequest()");
    OPENAUTO_LOG_INFO(ANDROID_AUTO, (std::stringstream() << "[MediaSourceService] Request to Open?: " << request.open() << ", anc: "
                       << request.anc_enabled() << ", ec: " << request.ec_enabled() << ", max unacked: "
                       << request.max_unacked()).str());
    ...
    if (request.open()) {
      ...
      startPromise->then(std::bind(&MediaSourceService::onMediaSourceOpenSuccess, this->shared_from_this()),
                         [this, self = this->shared_from_this()]() {
                           OPENAUTO_LOG_ERROR(ANDROID_AUTO, "[MediaSourceService] Media Source Open Failed");
  ...
  void MediaSourceService::onMediaSourceOpenSuccess() {
    OPENAUTO_LOG_ERROR(ANDROID_AUTO, "[MediaSourceService] onMediaSourceOpenSuccess()");
  ...
  void MediaSourceService::onMediaSourceDataReady(aasdk::common::Data data) {
    OPENAUTO_LOG_ERROR(ANDROID_AUTO, "[MediaSourceService] onMediaSourceDataReady()");
    auto sendPromise = aasdk::channel::SendPromise::defer(strand_);
  ...
  void MediaSourceService::readMediaSource() {
    OPENAUTO_LOG_DEBUG(ANDROID_AUTO, "[MediaSourceService] readMediaSource()");
    if (audioInput_->isActive()) {
      ...
      readPromise->then(
          std::bind(&MediaSourceService::onMediaSourceDataReady, this->shared_from_this(),
                    std::placeholders::_1),
          [this, self = this->shared_from_this()]() {
            OPENAUTO_LOG_DEBUG(ANDROID_AUTO, "[MediaSourceService] audio input read rejected.");
          });
