...
void InputSourceService::onChannelOpenRequest(
    const aap_protobuf::service::control::message::ChannelOpenRequest &request) {
    LOG_INFO(ANDROID_AUTO, "[InputSourceService] onChannelOpenRequest()");
    LOG_DEBUG(ANDROID_AUTO,
              ("[InputSourceService] Channel Id: " + std::to_string(request.service_id()) +
...
void InputSourceService::onKeyBindingRequest(
    ...
    LOG_DEBUG(ANDROID_AUTO,
              ("[InputSourceService] KeyCodes Count: " + std::to_string(request.keycodes_size()))
                  .c_str());
    ...
    for (int i = 0; i < request.keycodes_size(); ++i) {
        if (std::find(supportedButtonCodes.begin(), supportedButtonCodes.end(),
                      request.keycodes(i)) == supportedButtonCodes.end()) {
            LOG_ERROR(ANDROID_AUTO,
                      ("[InputSourceService] onKeyBindingRequest is not supported for KeyCode: " +
                       std::to_string(request.keycodes(i)))
                          .c_str());
    ...
    LOG_DEBUG(ANDROID_AUTO, ("[InputSourceService] Sending KeyBindingResponse with Status: " +
...
void InputSourceService::onChannelError(const aasdk::error::Error &e) {
    LOG_ERROR(ANDROID_AUTO,
              ("[InputSourceService] onChannelError(): " + std::string(e.what())).c_str());
}
...
void InputSourceService::onButtonEvent(const projection::ButtonEvent &event) {
    ...
    strand_.dispatch([this, self = this->shared_from_this(), event = std::move(event),
        ...
        if (event.code ==
            ...
            relativeEvent->set_delta(event.wheelDirection == projection::WheelDirection::LEFT ? -1
                                                                                              : 1);
        ...
        auto promise = aasdk::channel::SendPromise::defer(strand_);
        promise->then([]() {}, std::bind(&InputSourceService::onChannelError,
