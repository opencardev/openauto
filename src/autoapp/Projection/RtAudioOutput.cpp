...
bool RtAudioOutput::open() {
    ...
    if (dac_->getDeviceCount() > 0) {
        ...
        try {
            ...
            uint32_t bufferFrames = sampleRate_ == 16000
                                        ? 1024
            ...
            dac_->openStream(&parameters, nullptr, RTAUDIO_SINT16, sampleRate_, &bufferFrames,
                             &RtAudioOutput::audioBufferReadHandler, static_cast<void*>(this),
                             &streamOptions);
            LOG_INFO_STREAM(AUDIO, "[RtAudioOutput] Sample Rate: " + std::to_string(sampleRate_));
            return audioBuffer_.open(QIODevice::ReadWrite);
        } catch (const RtAudioError& e) {
            // TODO: Later version of RtAudio uses a different mechanism - FIXME - support new
            // versions
            LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] Failed to open audio output, what: " +
                                        std::string(e.what()));
        }
    } else {
