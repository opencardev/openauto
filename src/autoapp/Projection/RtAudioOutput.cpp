...
bool RtAudioOutput::open() {
    ...
    if (dac_->getDeviceCount() == 0) {
        LOG_ERROR(AUDIO, "[RtAudioOutput] No output devices found.");
        return false;
    }
    ...
    RtAudioErrorType err = dac_->openStream(
        ...
        RTAUDIO_SINT16, sampleRate_, &bufferFrames,
        &RtAudioOutput::audioBufferReadHandler, static_cast<void*>(this), &streamOptions);

    if (err != RTAUDIO_NO_ERROR)
    {
        LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] openStream failed, code=" + std::to_string(static_cast<int>(err))
    ...
    catch (const RtAudioError& e)
    {
        ...
        LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] Failed to open audio output, what: " + std::string(e.what()));
        return false;
    }
#endif

    LOG_INFO_STREAM(AUDIO, "[RtAudioOutput] Sample Rate: " + std::to_string(sampleRate_));
...
void RtAudioOutput::start() {
    ...
    if (dac_->isStreamOpen() && !dac_->isStreamRunning()) {
        ...
        if (err != RTAUDIO_NO_ERROR)
        {
            LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] startStream failed, code=" + std::to_string(static_cast<int>(err))
                                + " msg=" + dac_->getErrorText());
        }
#else
        try {
            ...
            LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] Failed to start audio output, what: " +
                                        std::string(e.what()));
        }
#endif
    }
...
void RtAudioOutput::doSuspend() {
    if (dac_->isStreamOpen() && dac_->isStreamRunning()) {
        ...
        if (err != RTAUDIO_NO_ERROR)
        {
            LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] stopStream failed, code=" + std::to_string(static_cast<int>(err))
                                + " msg=" + dac_->getErrorText());
        }
#else
        try {
            ...
            LOG_ERROR_STREAM(AUDIO, "[RtAudioOutput] Failed to suspend audio output, what: " +
                                        std::string(e.what()));
        }
#endif
    }
