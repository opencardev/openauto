...
void startUSBWorkers(boost::asio::io_service& ioService, libusb_context* usbContext,
...
void configureLogging() {
    ...
    if (debugMode) {
        ...
        logger.setCategoryLevel(openauto::modern::LogCategory::SYSTEM,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::UI,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::CAMERA,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::NETWORK,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::BLUETOOTH,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::AUDIO,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::VIDEO,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::CONFIG,
                                openauto::modern::LogLevel::DEBUG);
        logger.setCategoryLevel(openauto::modern::LogCategory::API,
                                openauto::modern::LogLevel::DEBUG);
    ...
    if (logSettings.good()) {
        SLOG_WARN(
            CONFIG, "autoapp",
            "Legacy log configuration file found - consider migrating to modern logger config");
    }
}
...
void validatePngAssets() {
    ...
    if (problematicCount == 0) {
        SLOG_INFO(UI, "autoapp",
                  "✅ All PNG assets validated successfully (" + std::to_string(validatedCount) +
                      " files)");
        ...
        SLOG_ERROR(UI, "autoapp",
                   "   📊 " + std::to_string(problematicCount) + " out of " +
                       std::to_string(pngResources.size()) + " PNG assets have issues");
        SLOG_ERROR(UI, "autoapp", "   ⚠️  This may cause 'libpng warning' messages at runtime");
        SLOG_ERROR(UI, "autoapp",
                   "   🔧 Rebuild the project to fix: cmake -B build && make -C build");
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::map<std::string, std::string> context = {{"display_width", std::to_string(width)},
                                                  {"display_height", std::to_string(height)}};
    SLOG_INFO(UI, "autoapp",
              "Display configuration: " + std::to_string(width) + "x" + std::to_string(height));
    ...
    try {
        ...
        SLOG_ERROR(SYSTEM, "autoapp",
                   "Failed to initialize modern components: " + std::string(e.what()));
    }
    ...
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openSettings, &settingsWindow,
                     &autoapp::ui::SettingsWindow::showFullScreen);
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openSettings, &settingsWindow,
                     &autoapp::ui::SettingsWindow::show_tab1);
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openSettings, &settingsWindow,
    ...
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openConnectDialog, &connectdialog,
                     &autoapp::ui::ConnectDialog::loadClientList);
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openConnectDialog, &connectdialog,
                     &autoapp::ui::ConnectDialog::exec);
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openUpdateDialog, &updatedialog,
                     &autoapp::ui::UpdateDialog::updateCheck);
    QObject::connect(&mainWindow, &autoapp::ui::MainWindow::openUpdateDialog, &updatedialog,
                     &autoapp::ui::UpdateDialog::exec);
    ...
    aasdk::usb::AccessoryModeQueryChainFactory queryChainFactory(usbWrapper, ioService,
                                                                 queryFactory);
    ...
    auto connectedAccessoriesEnumerator(
        std::make_shared<aasdk::usb::ConnectedAccessoriesEnumerator>(usbWrapper, ioService,
                                                                     queryChainFactory));
    auto app = std::make_shared<autoapp::App>(ioService, usbWrapper, tcpWrapper,
                                              androidAutoEntityFactory, std::move(usbHub),
                                              std::move(connectedAccessoriesEnumerator));

    QObject::connect(&connectdialog, &autoapp::ui::ConnectDialog::connectionSucceed,
    ...
    try {
        ...
        SLOG_ERROR(SYSTEM, "autoapp",
                   "Error during modern components shutdown: " + std::string(e.what()));
    }
