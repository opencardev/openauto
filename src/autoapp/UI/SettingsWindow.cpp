...
SettingsWindow::SettingsWindow(configuration::IConfiguration::Pointer configuration,
                               QWidget *parent)
    ...
    if (adapters.isEmpty()) {
        ...
        for (const QString &adapter : adapters) {
            ...
            if (parts.count() == 2) {
                ...
                comboBoxBluetooth->addItem(
                    QCoreApplication::translate("SettingsWindow", adapter.toUtf8().constData()),
                    QVariant(address));
            }
        }
    }
...
void SettingsWindow::onSave() {
    ...
    configuration_->setAudioOutputBackendType(ui_->radioButtonRtAudio->isChecked()
    ...
    params.append("#");
    params.append(std::string(
        ui_->comboBoxCountryCode->currentText().split("|")[0].replace(" ", "").toStdString()));
    params.append("#");
    ...
    params.append("#");
    params.append(
        std::string((ui_->comboBoxFPS->currentText()).replace(" (not @1080)", "").toStdString()));
    params.append("#");
    ...
    params.append("#");
    params.append(
        std::string(ui_->comboBoxUSBRotation->currentText().replace("180", "1").toStdString()));
    params.append("#");
...
void SettingsWindow::load() {
    ...
    ui_->radioButtonRtAudio->setChecked(audioOutputBackendType ==
                                        configuration::AudioOutputBackendType::RTAUDIO);
    ui_->radioButtonQtAudio->setChecked(audioOutputBackendType ==
                                        configuration::AudioOutputBackendType::QT);
    ...
    if (storage.isValid() && storage.isReady()) {
        if (storage.isReadOnly()) {
            ui_->labelStorage->setText(
                "Storage is read only!  (" + storage.device() +
                ") - This can be caused by demaged filesystem on CSSTORAGE. Try a reboot.");
        } else {
            ui_->labelStorage->setText(
                "Device: " + storage.device() + " Label: " + storage.displayName() +
                " Total: " + QString::number(storage.bytesTotal() / 1024 / 1024 / 1024) +
                "GB Free: " + QString::number(storage.bytesFree() / 1024 / 1024 / 1024) + "GB (" +
...
void SettingsWindow::saveButtonCheckBoxes() {
    ...
    this->saveButtonCheckBox(
        ...
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_MEDIA_PAUSE);
    this->saveButtonCheckBox(
        ui_->checkBoxTogglePlayButton, buttonCodes,
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_MEDIA_PLAY_PAUSE);
    this->saveButtonCheckBox(
        ui_->checkBoxNextTrackButton, buttonCodes,
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_MEDIA_NEXT);
    this->saveButtonCheckBox(
        ui_->checkBoxPreviousTrackButton, buttonCodes,
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_MEDIA_PREVIOUS);
    ...
    this->saveButtonCheckBox(
        ...
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_DPAD_DOWN);
    this->saveButtonCheckBox(
        ui_->checkBoxScrollWheelButton, buttonCodes,
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_ROTARY_CONTROLLER);
    this->saveButtonCheckBox(ui_->checkBoxBackButton, buttonCodes,
                             aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_BACK);
    this->saveButtonCheckBox(
        ui_->checkBoxEnterButton, buttonCodes,
        aap_protobuf::service::media::sink::message::KeyCode::KEYCODE_DPAD_CENTER);
    this->saveButtonCheckBox(
...
void SettingsWindow::saveButtonCheckBox(
    const QCheckBox *checkBox, configuration::IConfiguration::ButtonCodes &buttonCodes,
...
void SettingsWindow::onUpdateLux1(int value) { ui_->valueLux1->setText(QString::number(value)); }

void SettingsWindow::onUpdateLux2(int value) { ui_->valueLux2->setText(QString::number(value)); }

void SettingsWindow::onUpdateLux3(int value) { ui_->valueLux3->setText(QString::number(value)); }

void SettingsWindow::onUpdateLux4(int value) { ui_->valueLux4->setText(QString::number(value)); }

void SettingsWindow::onUpdateLux5(int value) { ui_->valueLux5->setText(QString::number(value)); }
...
void SettingsWindow::unpairAll() { system("/usr/local/bin/crankshaft bluetooth unpair &"); }
...
void SettingsWindow::syncNTPTime() { system("/usr/local/bin/crankshaft rtc sync &"); }
...
void SettingsWindow::loadSystemValues() {
    ...
    if (std::ifstream("/tmp/return_value")) {
        ...
        ui_->spinBoxDisconnect->setValue(
            configuration_->getCSValue("DISCONNECTION_SCREEN_POWEROFF_SECS").toInt());
        // set day/night
        ...
        // set rtc
        QString rtcstring =
            configuration_->getParamFromFile("/boot/config.txt", "dtoverlay=i2c-rtc");
        if (rtcstring != "") {
        ...
        if (configuration_->getCSValue("ENABLE_BLUETOOTH") == "1") {
            QString bt =
                configuration_->getParamFromFile("/boot/config.txt", "dtoverlay=pi3-disable-bt");

            QString selectedAddress = QString::fromStdString(
                configuration_->getBluetoothAdapterAddress());  //.value("BluetoothAdapterAddress",
                                                                //"none").toString();
        ...
        // wifi country code
        ui_->comboBoxCountryCode->setCurrentIndex(ui_->comboBoxCountryCode->findText(
            configuration_->getCSValue("WIFI_COUNTRY"), Qt::MatchFlag::MatchStartsWith));
...
void SettingsWindow::getMacMemoryInfo(QString &freeMemory) {
    ...
    if (host_statistics64(host_port, HOST_VM_INFO64, (host_info64_t)&vm_stats, &count) !=
        KERN_SUCCESS) {
...
void SettingsWindow::updateSystemInfo() {
    ...
    // current cpu speed
    QString freq =
        configuration_->readFileContent("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_cur_freq");
