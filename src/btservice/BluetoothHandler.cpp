...
namespace f1x::openauto::btservice {
  ...
  void BluetoothHandler::onPairingFinished(const QBluetoothAddress &address, QBluetoothLocalDevice::Pairing pairing) {
    LOG_INFO_STREAM(BLUETOOTH, "[BluetoothHandler::onPairingFinished] pairingFinished, address: " << address.toString().toStdString()
                       << ", pairing: " << pairing);
  }

  void BluetoothHandler::onError(QBluetoothLocalDevice::Error error) {
    LOG_WARN_STREAM(BLUETOOTH, "[BluetoothHandler::onError] Bluetooth error: " << error);
    // ... your logic to handle the error ...
  }

  void BluetoothHandler::onHostModeStateChanged(QBluetoothLocalDevice::HostMode state) {
    LOG_INFO_STREAM(BLUETOOTH, "[BluetoothHandler::onHostModeStateChanged] Host mode state changed: " << state);
