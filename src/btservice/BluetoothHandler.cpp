...
BluetoothHandler::BluetoothHandler(
    btservice::IAndroidBluetoothService::Pointer androidBluetoothService,
    ...
    QObject::connect(localDevice_.get(), &QBluetoothLocalDevice::pairingDisplayPinCode, this,
                     &BluetoothHandler::onPairingDisplayPinCode);
    QObject::connect(localDevice_.get(), &QBluetoothLocalDevice::pairingDisplayConfirmation, this,
                     &BluetoothHandler::onPairingDisplayConfirmation);
    QObject::connect(localDevice_.get(), &QBluetoothLocalDevice::pairingFinished, this,
                     &BluetoothHandler::onPairingFinished);
    QObject::connect(localDevice_.get(), &QBluetoothLocalDevice::hostModeStateChanged, this,
                     &BluetoothHandler::onHostModeStateChanged);
    QObject::connect(localDevice_.get(), &QBluetoothLocalDevice::error, this,
                     &BluetoothHandler::onError);
    ...
    std::map<std::string, std::string> context = {{"address", address.toString().toStdString()},
    ...
    if (!androidBluetoothService_->registerService(portNumber, address)) {
        ...
        std::map<std::string, std::string> serviceContext = {{"port", std::to_string(portNumber)}};
...
void BluetoothHandler::onPairingDisplayPinCode(const QBluetoothAddress &address, QString pin) {
    LOG_DEBUG_STREAM(BLUETOOTH,
                     "[BluetoothHandler::onPairingDisplayPinCode] Pairing display PIN code: " +
                         pin.toStdString());
}
...
void BluetoothHandler::onPairingDisplayConfirmation(const QBluetoothAddress &address, QString pin) {
    LOG_DEBUG_STREAM(
        BLUETOOTH,
        "[BluetoothHandler::onPairingDisplayConfirmation] Pairing display confirmation: " +
            pin.toStdString());

    // Here you can implement logic to show this PIN to the user or automatically accept if you
    // trust all devices
    localDevice_->pairingConfirmation(
        true);  // Confirm pairing (for security, you might want to verify the PIN)
}

void BluetoothHandler::onPairingFinished(const QBluetoothAddress &address,
                                         QBluetoothLocalDevice::Pairing pairing) {
    LOG_INFO_STREAM(BLUETOOTH, "[BluetoothHandler::onPairingFinished] pairingFinished, address: " +
                                   address.toString().toStdString() +
                                   ", pairing: " + std::to_string(pairing));
}

void BluetoothHandler::onError(QBluetoothLocalDevice::Error error) {
    LOG_WARN_STREAM(BLUETOOTH,
                    "[BluetoothHandler::onError] Bluetooth error: " + std::to_string(error));
    // ... your logic to handle the error ...
}

void BluetoothHandler::onHostModeStateChanged(QBluetoothLocalDevice::HostMode state) {
    LOG_INFO_STREAM(BLUETOOTH,
                    "[BluetoothHandler::onHostModeStateChanged] Host mode state changed: " +
                        std::to_string(state));
