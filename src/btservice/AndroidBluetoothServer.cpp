...
AndroidBluetoothServer::AndroidBluetoothServer(
    ...
    : rfcommServer_(
          std::make_unique<QBluetoothServer>(QBluetoothServiceInfo::RfcommProtocol, this)),
...
void AndroidBluetoothServer::readSocket() {
    ...
    std::map<std::string, std::string> context = {{"message_length", std::to_string(length)},
                                                  {"message_id", std::to_string(messageId)}};
    LOG_DEBUG_CTX(BLUETOOTH, "Processing message", context);

    switch (messageId) {
        case aap_protobuf::aaw::MessageId::WIFI_INFO_REQUEST:  // WifiInfoRequest - Respond with a
                                                               // WifiInfoResponse
            handleWifiInfoRequest(buffer, length);
            break;
        case aap_protobuf::aaw::MessageId::WIFI_VERSION_RESPONSE:  // WifiVersionRequest - Send a
                                                                   // Version Request
            handleWifiVersionResponse(buffer, length);             // do something
            break;
        case aap_protobuf::aaw::MessageId::WIFI_CONNECTION_STATUS:  // WifiStartResponse  - Receive
                                                                    // a confirmation
            handleWifiConnectionStatus(buffer, length);
            break;
        case aap_protobuf::aaw::MessageId::WIFI_START_RESPONSE:  // WifiStartResponse  - Receive a
                                                                 // confirmation
            handleWifiStartResponse(buffer, length);
            break;
        case aap_protobuf::aaw::MessageId::WIFI_START_REQUEST:    // These are not received from the
                                                                  // MD.
        case aap_protobuf::aaw::MessageId::WIFI_INFO_RESPONSE:    // These are not received from the
                                                                  // MD.
        case aap_protobuf::aaw::MessageId::WIFI_VERSION_REQUEST:  // These are not received from the
                                                                  // MD.
        default:
            ...
            std::map<std::string, std::string> context = {{"message_id", std::to_string(messageId)},
                                                          {"data", ss.str()}};
...
void AndroidBluetoothServer::handleWifiStartResponse(QByteArray &buffer, uint16_t length) {
    ...
    response.ParseFromArray(buffer.data() + 4, length);
    std::map<std::string, std::string> context = {{"ip_address", response.ip_address()},
...
void AndroidBluetoothServer::handleWifiConnectionStatus(QByteArray &buffer, uint16_t length) {
    ...
    std::map<std::string, std::string> context = {{"status", Status_Name(status.status())}};
...
void AndroidBluetoothServer::sendMessage(const google::protobuf::Message &message, uint16_t type) {
    ...
    std::map<std::string, std::string> context = {{"message_type", message.GetTypeName()},
                                                  {"debug_string", message.DebugString()}};
    LOG_DEBUG_CTX(BLUETOOTH, "Sending protobuf message", context);

    auto written = socket->write(out);
    if (written > -1) {
        std::map<std::string, std::string> context = {{"bytes_written", std::to_string(written)}};
...
const ::std::string AndroidBluetoothServer::getIP4_(const QString intf) {
    for (const QNetworkAddressEntry &address :
         QNetworkInterface::interfaceFromName(intf).addressEntries()) {
    ...
    for (int i = 0; i < set.field_count(); ++i) {
        ...
        switch (field.type()) {
            case UnknownField::TYPE_VARINT:
                std::cout << "Field number " << field.number() << " is a varint: " << field.varint()
                          << std::endl;
                break;
            case UnknownField::TYPE_FIXED32:
                std::cout << "Field number " << field.number()
                          << " is a fixed32: " << field.fixed32() << std::endl;
                break;
            case UnknownField::TYPE_FIXED64:
                std::cout << "Field number " << field.number()
                          << " is a fixed64: " << field.fixed64() << std::endl;
                break;
