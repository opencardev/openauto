    ...
    , server_(std::make_unique<httplib::Server>())
{
    ...
    Logger::info("RestApiServer initialized on port " + std::to_string(port_));
}
...
bool RestApiServer::start() {
    ...
    serverThread_ = std::thread([this]() {
        running_ = true;
        Logger::info("Starting OpenAuto REST API server on " + bindAddress_ + ":" + std::to_string(port_));
        
        if (!server_->listen(bindAddress_, port_)) {
            Logger::error("Failed to start REST API server on port " + std::to_string(port_));
            running_ = false;
        }
    });
...
void RestApiServer::stop() {
    ...
    Logger::info("Stopping OpenAuto REST API server");
    running_ = false;
...
void RestApiServer::setupDefaultMiddleware() {
    ...
    addGlobalMiddleware([](HttpRequest& req, HttpResponse& res) -> bool {
        Logger::info("REST API request: " + methodToString(req.getMethod()) + " " + req.getPath());
...
void RestApiServer::registerRoute(const Route& route) {
    auto handler = [this, route](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            Logger::error("Error in route handler: " + std::string(e.what()));
...
std::string RestApiServer::methodToString(HttpMethod method) {
    switch (method) {
