...
size_t Logger::getDroppedMessages() const { return droppedMessages_.load(); }

void Logger::processLogs() {
    while (!shutdown_.load()) {
        std::unique_lock<std::mutex> lock(mutex_);

        condition_.wait(lock, [this] { return !logQueue_.empty() || shutdown_.load(); });
...
std::string ConsoleFormatter::format(const LogEntry& entry) {
    ...
    // Level with color
    if (useColors_) {
        oss << " " << getLevelColor(entry.level) << "[" << Logger::levelToString(entry.level) << "]"
            << resetColor();
    } else {
    ...
    // Category with color
    if (useColors_) {
        oss << " " << getCategoryColor(entry.category) << "["
            << Logger::categoryToString(entry.category) << "]" << resetColor();
    } else {
...
std::string ConsoleFormatter::resetColor() const { return useColors_ ? "\033[0m" : ""; }
...
std::string JsonFormatter::format(const LogEntry& entry) {
    ...
    std::ostringstream oss;
    oss << "{\"level\":\"" << Logger::levelToString(entry.level) << "\",\"category\":\""
        << Logger::categoryToString(entry.category) << "\",\"message\":\"" << entry.message
        << "\"}\n";
    return oss.str();
#endif
}
