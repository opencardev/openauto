...
RestApiServer::RestApiServer(int port, std::shared_ptr<EventBus> eventBus,
    ...
    Logger::getInstance().info(LogCategory::GENERAL, "RestApiServer", __FUNCTION__, __FILE__,
                               __LINE__, "RestApiServer created on port " + std::to_string(port));
}
...
bool RestApiServer::start() {
    if (running_) {
        Logger::getInstance().warn(LogCategory::API, "RestApiServer", __FUNCTION__, __FILE__,
                                   __LINE__, "Server already running");
        return true;
    }

    Logger::getInstance().info(LogCategory::API, "RestApiServer", __FUNCTION__, __FILE__, __LINE__,
                               "Starting REST API server on port " + std::to_string(port_));
    ...
    impl_->serverThread = std::thread([this]() {
        try {
            Logger::getInstance().info(
                LogCategory::API, "RestApiServer", "serverThread", __FILE__, __LINE__,
                "HTTP server listening on " + bindAddress_ + ":" + std::to_string(port_));
            impl_->server->listen(bindAddress_.c_str(), port_);
        } catch (const std::exception& e) {
            Logger::getInstance().error(LogCategory::API, "RestApiServer", "serverThread", __FILE__,
                                        __LINE__, "Server error: " + std::string(e.what()));
            running_ = false;
        }
    });
...
void RestApiServer::setPort(int port) { port_ = port; }

int RestApiServer::getPort() const { return port_; }

void RestApiServer::setBindAddress(const std::string& address) { bindAddress_ = address; }

std::string RestApiServer::getBindAddress() const { return bindAddress_; }

void RestApiServer::setApiInfo(const ApiInfo& info) { apiInfo_ = info; }

ApiInfo RestApiServer::getApiInfo() const { return apiInfo_; }

void RestApiServer::addServer(const ServerInfo& server) { servers_.push_back(server); }

void RestApiServer::addSecurityScheme(const std::string& name, const SecurityScheme& scheme) {
    securitySchemes_[name] = scheme;
}

void RestApiServer::addRoute(const Route& route) { routes_.push_back(route); }
...
void RestApiServer::enableSwaggerUI(const std::string& path) {
    Logger::getInstance().info(LogCategory::GENERAL, "RestApiServer", __FUNCTION__, __FILE__,
                               __LINE__, "SwaggerUI enabled at: " + path);
}

void RestApiServer::enableReDoc(const std::string& path) {
    Logger::getInstance().info(LogCategory::GENERAL, "RestApiServer", __FUNCTION__, __FILE__,
                               __LINE__, "ReDoc enabled at: " + path);
}
...
void RestApiServer::setupDefaultRoutes() {
    ...
    impl_->server->Get("/info", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        info["description"] = apiInfo_.description.empty()
                                  ? "REST API for OpenAuto Android Auto implementation"
                                  : apiInfo_.description;
...
void RestApiServer::setupApiRoutes() {
    // Events endpoints
    impl_->server->Get(
        "/api/v1/events", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        "/api/v1/events", [this](const httplib::Request& req, httplib::Response& res) {
            try {
                ...
                if (!body.contains("type")) {
                    auto response =
                        createErrorResponse(400, "Bad Request", "Missing required field: type");
                    res.set_content(response.dump(), "application/json");
                    res.status = 400;
                    return;
                }
                ...
                auto response = createErrorResponse(400, "Bad Request",
    ...
    impl_->server->Get(
        "/api/v1/events/types", [this](const httplib::Request& req, httplib::Response& res) {
            nlohmann::json eventTypes = nlohmann::json::array({"SYSTEM_STARTUP",
                                                               "SYSTEM_SHUTDOWN",
                                                               "SYSTEM_REBOOT",
                                                               "SYSTEM_ERROR",
    ...
    // State endpoints
    impl_->server->Get("/api/v1/state",
                       [this](const httplib::Request& req, httplib::Response& res) {
                           nlohmann::json state;

                           if (stateMachine_) {
                               // state["current"] = stateMachine_->getCurrentState(); // Would need
                               // to implement this
                           ...
                           state["available_states"] =
                               nlohmann::json::array({"idle", "connected", "projection", "error"});
    ...
    impl_->server->Post(
        "/api/v1/state/transition", [this](const httplib::Request& req, httplib::Response& res) {
            try {
                auto body = nlohmann::json::parse(req.body);

                if (!body.contains("state")) {
                    auto response =
                        createErrorResponse(400, "Bad Request", "Missing required field: state");
                    res.set_content(response.dump(), "application/json");
                    res.status = 400;
                    return;
                }
    ...
    impl_->server->Get(
        "/api/v1/state/history", [this](const httplib::Request& req, httplib::Response& res) {
    ...
    impl_->server->Get("/api/v1/config",
                       [this](const httplib::Request& req, httplib::Response& res) {
                           nlohmann::json config;

                           if (configManager_) {
                               // config = configManager_->getAllConfig(); // Would need to
                               // implement this Mock configuration
    ...
    impl_->server->Get(
        R"(/api/v1/config/(.+))", [this](const httplib::Request& req, httplib::Response& res) {
            ...
            if (configManager_) {
                ...
                if (key == "ui.brightness")
                    ...
                    auto response = createErrorResponse(404, "Not Found",
                ...
                auto response = createErrorResponse(503, "Service Unavailable",
        ...
        R"(/api/v1/config/(.+))", [this](const httplib::Request& req, httplib::Response& res) {
            ...
            try {
                ...
                if (!body.contains("value")) {
                    auto response =
                        createErrorResponse(400, "Bad Request", "Missing required field: value");
                    res.set_content(response.dump(), "application/json");
                    res.status = 400;
                    return;
                }
    ...
    impl_->server->Post(
        "/api/v1/config/save", [this](const httplib::Request& req, httplib::Response& res) {
            if (configManager_) {
...
std::string HttpRequest::getPath() const { return request_ ? request_->path : "/"; }
...
std::string HttpRequest::getBody() const { return request_ ? request_->body : ""; }
...
int HttpResponse::getStatus() const { return statusCode_; }
...
std::string HttpResponse::getContentType() const { return contentType_; }
